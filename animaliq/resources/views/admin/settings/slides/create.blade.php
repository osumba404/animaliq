...
<form action="{{ route('admin.settings.slides.store') }}" method="POST" class="max-w-md">
    ...
    <div class="mb-4"><label class="block font-medium theme-text-secondary mb-1">CTA text</label><input type="text" name="cta_text" value="{{ old('cta_text') }}" class="theme-input w-full"></div>
    <div class="mb-4"><label class="block font-medium theme-text-secondary mb-1">CTA link</label><input type="text" name="cta_link" value="{{ old('cta_link') }}" class="theme-input w-full"></div>
