...
@endphp
<div class="flex gap-3 mb-6 comment-item" id="comment-{{ $comment->id }}">
    ...
    <div class="flex-1">
        ...
        <div class="flex items-center gap-3 mt-2">
            <button class="like-comment-btn flex items-center gap-1 text-xs theme-text-secondary hover:text-red-500 transition {{ $isLiked ? '' : '' }}" style="{{ $isLiked ? 'color:var(--accent-orange)' : '' }}" data-id="{{ $comment->id }}">
            ...
            @auth
            <button class="reply-btn text-xs theme-text-secondary hover:theme-accent transition" data-id="{{ $comment->id }}" data-name="{{ $comment->user->first_name }}">Reply</button>
            @endauth
        </div>

        {{-- Replies --}}
        @if($comment->replies->isNotEmpty())
        <div class="replies-list mt-4 space-y-4 ml-2 border-l-2 pl-4" style="border-color:var(--border-color)">
            ...
            <div class="flex gap-2 comment-item" id="comment-{{ $reply->id }}">
                ...
                <div class="flex-1">
                    ...
                    <button class="like-comment-btn flex items-center gap-1 text-xs theme-text-secondary hover:text-red-500 transition mt-1" style="{{ $replyLiked ? 'color:var(--accent-orange)' : '' }}" data-id="{{ $reply->id }}">
